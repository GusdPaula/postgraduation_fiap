...
class InfoPage():
    ...
    def __init__(self, lang):
        ...
        if lang == "EN":
            ...
            st.write('O Streamlit permite implantar e hospedar facilmente aplicativos de dados interativos. Aqui estão as etapas detalhadas e considerações para implantar esse dashboard: ')
            st.write('1. **Preparar o Código e Dependências:**')
            st.write('   - Certifique-se de que todos os arquivos de código estejam devidamente estruturados e enviados para um repositório do GitHub.')
            st.write('   - Certifique-se de que todas as bibliotecas e dependências necessárias estejam listadas no arquivo **requirements.txt**.')
            st.write('   - Certifique-se de que o ponto de entrada do aplicativo seja **app.py** ou um arquivo de nome semelhante que contenha a lógica do Streamlit para iniciar o dashboard.')
            st.write('2. **Repositório do GitHub:**')
            st.write('   - O código deste projeto deve ser armazenado em um repositório público ou privado no GitHub, conforme sua preferência.')
            st.write('   - Certifique-se de que o repositório contenha o código mais recente, incluindo **app.py**, **requirements.txt**, e outros arquivos necessários para a funcionalidade do dashboard.')
            ...
            st.write('   - Conecte sua conta do GitHub ao Streamlit Cloud.')
            st.write('   - A partir do Streamlit Cloud, crie um novo aplicativo e selecione o repositório GitHub que contém o código do seu dashboard.')
            st.write('4. **Configuração de Implantação:**')
            st.write('   - Assim que seu repositório estiver vinculado, o sistema verificará automaticamente a presença de um arquivo **app.py** ou o ponto de entrada correto e instalará as dependências do **requirements.txt**.')
            ...
            st.write('   - Após o Streamlit implantar o aplicativo, você receberá uma URL pública para o dashboard. Acesse essa URL e verifique a funcionalidade do aplicativo.')
            st.write('   - Se você encontrar erros ou problemas, revise os logs do Streamlit para depuração e resolução de problemas.')
            st.write('   - É recomendado também testar o aplicativo em diferentes navegadores e dispositivos para garantir compatibilidade entre plataformas.')
